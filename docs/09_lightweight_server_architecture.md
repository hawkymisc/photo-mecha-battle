...
[`docs/00`](00_root_overview.md)〜`08` と矛盾する記述がある場合、**本書の分担・API方針を優先する**。ゲームルール（バトル式、型相性、課金公平性、情報量スコアの意味など）は [`docs/03`](03_mech_generation_and_stats.md)〜`06` を引き続き正本とする。
...
| バトルはサーバー権威 | 非同期PvPの公平性のため、勝敗・ログ・seed はサーバーで確定する（[`docs/05`](05_team_and_battle.md)） |
| 性能はクライアント由来、権威はサーバー | 特徴量・ステータスはクライアントで算出してよいが、サーバーが同じ式で検証してから確定する |
| 見た目と戦闘性能の分離 | 生成画像は見た目のみ。戦闘性能は分析値から決定する（[`docs/00`](00_root_overview.md) 原則2） |
...
| 領域 | 内容 | 備考 |
|---|---|---|
| 撮影UX | プレビュー、被写体ガイド、明るさ・ブレ警告 | [`docs/02`](02_photo_object_extraction.md) |
| 物体検出 | 候補 bbox / マスクの提示 | ユーザーが候補選択・タップ補正 |
| セグメント | 背景分離、マスク後処理（穴埋め・ノイズ除去） | 手動矩形・タップ補正はクライアント |
| 特徴量 | `FeatureVector` の算出、情報量スコアのプレビュー | 式は [`docs/03`](03_mech_generation_and_stats.md) とサーバー実装と一致させる |
| 型推定（プレビュー） | `form_inference/1.0` による型の事前表示 | 最終確定はサーバー。クライアントは `form` を送信しない |
| メカ見た目（MVP） | 推定された型に基づくクロップへの簡易スタイライズ | 戦闘性能に影響しない |
...
| バトル確定 | seed 生成、シミュレーション、勝敗・ログ保存 | サーバー権威（[`docs/05`](05_team_and_battle.md)） |
| ランキング | レーティング更新 | |
| 不正対策（最終） | perceptual hash、クォータ、特徴量の再検証 | クライアント単独は信用しない |
| メカ型・ステータス | `form_inference/1.0` による型推定、`MechStats` 確定 | クライアント送信の `form` は無視 |
| 課金（最終） | Webhook 受信、Entitlement 保持 | クライアントのみで確定しない（[`docs/07`](07_platform_and_system.md)） |
...
| バトル演出・エフェクト | クライアント |
| 自然言語戦術LLM（将来） | 原則クライアント（ローカルLLM）。[`docs/06`](06_monetization_and_fairness.md) |
...
```text
1. クライアントが crop 画像 + feature_vector + bbox + メタデータを送信（`form` は含めない）
2. サーバーが以下を実行
...
sequenceDiagram
    ...
    C->>C: 撮影・検出・セグメント・特徴量
    C->>C: 型推定プレビュー・見た目プレビュー
    C->>S: POST /mechs（crop, features, bbox, name, algo_version）
    S->>S: phash / クォータ / 特徴量再計算
    S->>S: form 推定（form_inference/1.0）・stats 確定・保存
    S-->>C: mech_id, form, stats, art_url（任意）
```
...
> **実装状態**: `POST /battles/ranked` の seed はサーバー生成（`secrets.randbits`）。クライアント送信
...
| メソッド | パス | 担当 | 内容 |
|---|---|---|---|
| POST | `/mechs` | サーバー（検証+保存） | crop・features を受け取り form 推定・stats 確定 |
...
| POST | `/billing/entitlements` | デモ用 Entitlement 付与。**本番では管理者限定必須**（[`docs/06`](06_monetization_and_fairness.md)） |
...
[`docs/07`](07_platform_and_system.md) の候補から選ぶ。軽量サーバー前提では以下が相性がよい。
...
環境（local / staging / production）と本番公開前ブロッカー（ファイル SQLite 永続化など）は [`docs/12`](12_environments.md) を正とする。

## テスト方針

| 層 | 内容 |
|---|---|
| 共有ロジック | 特徴量・info_score・型推定・バトルエンジンは同一入力でクライアント/サーバー一致テスト |
...
以下は [`docs/00`](00_root_overview.md)〜`08` の正本を維持する。

- 3体チーム・前中後衛・型相性
- 特徴量からのメカ型自動推定（`form_inference/1.0`）
...
| ドキュメント | 関係 |
|---|---|
| [`docs/02`](02_photo_object_extraction.md) | 撮影UX・品質評価の観点（クライアント実装） |
| [`docs/03`](03_mech_generation_and_stats.md) | FeatureVector・型推定・info_score・MechStats の定義 |
| [`docs/05`](05_team_and_battle.md) | サーバー権威バトル・クライアント演出 |
| [`docs/06`](06_monetization_and_fairness.md) | 課金公平性・ローカルLLM |
| [`docs/07`](07_platform_and_system.md) | 認証・データモデル・API 共通規約（構成図・API 一覧は本書に一本化済み） |
| [`docs/08`](08_mvp_and_roadmap.md) | MVP フェーズ（クライアント未実装分は引き続き残タスク） |
| [`docs/12`](12_environments.md) | local / staging / production の分離・シークレット・デプロイ対応 |
