...
> **確定状態（2026-07-02 更新）**: システム構成図と API 一覧は `docs/09_lightweight_server_architecture.md`
> に一本化した（本書の旧「サーバー集中型 ML パイプライン」構成は廃止）。
> データモデルは現行実装（`api/database.py` の SQLite スキーマ）と同期済み。
...
軽量サーバー・クライアント厚め構成（`docs/09`）では Flutter またはネイティブが第一候補。
バックエンド言語（Python）はサーバー専用であり、アプリ本体を Python にする必要はない。

## システム構成・API

**`docs/09_lightweight_server_architecture.md` を正とする。** 本書では重複定義しない。

- 全体構成図・責務分担 → `docs/09` 「全体構成」「責務分担」
- API 一覧（主経路・互換用の区別含む）→ `docs/09` 「API 設計」
- 信頼モデル（クライアント算出値の検証）→ `docs/09` 「信頼モデル」
...
| 404 | リソース不存在 |
| 409 | 重複撮影（perceptual hash 判定。`docs/02`） |
| 429 | 日次クォータ超過（`docs/06`） |

### 所有権

- 戦術・チームはユーザーに紐づく。他ユーザーのチーム参照・更新・出撃は 403 で拒否する。
- バトル結果・ログは**対戦当事者のみ**参照可能とする。
  ※ 現行実装の `GET /battles/{id}` は未認証で参照できる既知の乖離がある（PLAN D-008 参照）。

## データモデル（現行実装と同期）

ストレージは MVP では SQLite（`api/database.py`）。カラムの意味を変えずに PostgreSQL へ移行可能な設計とする。
...
| rating | レーティング（初期 1000、下限 0。更新規則は `docs/05`） |
...
| perceptual_hash | 8×8 平均ハッシュ（64bit hex。`docs/02`） |
| safety_status | `ok` / `warning`（`docs/02` 品質評価） |
...
| features_json | 特徴量ベクトル 11 次元（`docs/03`） |
| info_score | 情報量スコア（サーバー確定値） |
| detected_label | 検出ラベル |
| confidence | マスク信頼度 |
| quality_json | 品質スコア一式（`docs/02`） |
| safety_status | 安全性判定 |
...
| name | 機体名 |
| stats_json | 確定ステータス（HP/ATK/DEF/SPD/TEC/EN/LUCK。算出は `docs/03`） |
| art_url | 生成アート URL（見た目のみ。戦闘性能に影響しない） |

※ `rarity` / `skills` は将来拡張（`docs/03` 将来拡張参照）。現行スキーマには存在しない。
...
| id | UUID |
| user_id | 所有ユーザー |
| payload_json | `{name, slots[{condition{kind, threshold}, action}], fallback_action}`（`docs/04`） |
...
| queued | マッチングキュー登録フラグ（`docs/05` マッチング参照） |
...
※ ログは構造化 JSON 保存へ移行予定（`docs/05` バトルログ参照。現行は整形テキスト）。
...
|---|---|
| user_id | 対象ユーザー |
| entitlement_key | Entitlement キー（`docs/06` の定義表） |
| is_active | 有効フラグ |
...
上限値と premium 拡大は `docs/06` 「生成クォータ」を正とする。

## RevenueCat 連携

役割分担・Entitlement 定義・Webhook 処理は `docs/06`、API パスは `docs/09` を正とする。
