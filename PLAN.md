...
| ✅ | D-010 | ダメージ式にスケーリング係数が無く全バトルが引き分け | [docs/05](docs/05_team_and_battle.md) ダメージ式（確定）| **実装済み（案A）**: `DAMAGE_SCALING = 60` を分子に導入 `ATK×SkillPower×K/(DEF+100)`（[`battle.py`](src/photo_mecha_battle/battle.py)）。分母の `+100` はソフトアーマー正規化定数として維持。キャリブレーション例 ATK=80,DEF=50→32dmg を docs/05 表示例と整合させテスト固定。実ステータス帯での決着可能性（20 seed 中 15 以上決着）・最低ダメージ床からの脱出・同一 seed 決定性を [`tests/test_damage_scaling.py`](tests/test_damage_scaling.py) で検証。docs/05 に K と +100 の意味注記を追記。※ Issue #23 |
| 🔲 | D-014 | `form_inference_version` の永続化 | [docs/03](docs/03_mech_generation_and_stats.md) 型推定ルール | `mechs` テーブルに版数列を追加（`ALTER TABLE`、D-003 と同パターン）し作成時の版数を保存。旧レコードは `null` を返す。`form_inference/2.0` 着手前が期限。※ Issue #31、D-013 の Codex レビュー中に発見 |
| ✅ | D-013 | メカ型の自動推定 | [docs/03](docs/03_mech_generation_and_stats.md) 型推定ルール | **実装済み**: `infer_form(features)` を `form_inference/1.0` で実装（[`mech_stats.py`](src/photo_mecha_battle/mech_stats.py)）。`POST /mechs` はクライアント `form` を受理するが無視し（後方互換のため 400 にはしない、D-007 と同型）、サーバー推定で確定。応答に `form_inference_version` を含める。PoC Web は型ラジオボタンを撤去し生成後に判明型を表示。代表例（傘→bird・石→beast・ペン→human）・同点タイブレーク（human>bird>beast、1e-9）・境界値（全 0.0/1.0）をテスト固定（[`tests/test_form_inference.py`](tests/test_form_inference.py)）。`vertical_slice.py` も自動推定デモに更新。※ PO 決定済み（`config/po_pending_decisions.json` form_inference_policy） |
...
## Phase E — 環境・運用（docs/12）

[`docs/12`](docs/12_environments.md) の環境分離。QA は `staging` 共用。ホスティング先は PO 未決（E-001〜E-005）。

| Status | ID | Task | Notes |
|---|---|---|---|
| ✅ | E-000 | 環境設計ドキュメント | [`docs/12`](docs/12_environments.md)。local / staging / production |
| ✅ | E-010 | SQLite ファイル永続化（`PMB_DB_PATH`） | 既定 `{PMB_DATA_DIR}/pmb.sqlite3`。テストは `:memory:` 差し替え |
| 🔲 | E-011 | `PMB_ENV` 起動時ガード | 後回し。production で admin トークン誤設定・`/app` 誤配信を起動失敗にする |
| 🔲 | E-012 | staging 環境の立ち上げ | 候補 Cloudflare（アカウント未）。FastAPI は Containers 等を要検証 |
| 🔲 | E-013 | クライアント staging/production フレーバー | **後回し**。当面はビルド引数で API URL 差し替え |
...
- [`config/po_pending_decisions.json`](config/po_pending_decisions.json): バトル・画像生成・戦術生成・課金の未決事項（[docs/08](docs/08_mvp_and_roadmap.md#未決事項) 対応）。画像生成モデル調査は [docs/10](docs/10_mobile_image_generation_survey.md)
- [`config/revenuecat_pending_setup.json`](config/revenuecat_pending_setup.json): RevenueCat ダッシュボード側の外部設定（D-005 の残課題）
- [`docs/12`](docs/12_environments.md) E-001〜E-005: ホスティング先・staging 常時起動・ドメイン・バックアップ目標・内部配布手段
