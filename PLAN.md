...
| 🔲 | D-013 | メカ型の自動推定 | [docs/03](docs/03_mech_generation_and_stats.md) 型推定ルール | `infer_form(features)` を `form_inference/1.0` で実装。`POST /mechs` はクライアント `form` を無視しサーバー推定で確定。PoC Web の型ラジオボタンを判明型表示に変更。傘→bird・石→beast 等の代表例をテスト固定。※ PO 決定済み（`config/po_pending_decisions.json` form_inference_policy） |
