...
| 🔲 | D-010 | ダメージ式にスケーリング係数が無く全バトルが引き分け | [docs/05](docs/05_team_and_battle.md) ダメージ式（確定）| **PO承認済み（案A・実装待ち）**: 分子にスケーリング係数 K を導入 `ATK×SkillPower×K/(DEF+100)`。分母の `+100` はソフトアーマー正規化定数として**維持**（基本装甲100ではない。Issue #23 調査結果参照）。K は docs/05 ログ例と実ステータス帯でキャリブレーション（目安 K=60〜80、例: ATK=80,DEF=50,K=60→≈32dmg）。現状は素点が常に1.0未満で全打撃1ダメージ化→20戦全引き分け→ランキング死文化。docs/05（式・+100注記・ログ例）・テスト（現実的ステータス帯での決着可能性、決定性維持）の同時更新必須。※ Issue #23 |
