...
| 🔲 | E-010 | SQLite ファイル永続化（`PMB_DB_PATH`） | 現行 `Database(":memory:")` は再起動で消失。staging 立ち上げ前のブロッカー |
| 🔲 | E-011 | `PMB_ENV` 起動時ガード | production で admin トークン誤設定・`/app` 誤配信を起動失敗にする |
| 🔲 | E-012 | staging 環境の立ち上げ | ホスティング先は E-001（PO）。HTTPS + 永続ボリューム |
| 🔲 | E-013 | クライアント staging/production フレーバー | API URL 取り違え防止（Android flavor / iOS xcconfig） |
