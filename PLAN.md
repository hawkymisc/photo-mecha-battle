...
| 🔲 | D-010 | ダメージ式にスケーリング係数が無く全バトルが引き分け | [docs/05](docs/05_team_and_battle.md) ダメージ式（確定）| **要PO判断（バトル式の破壊的変更に該当）**: `ATK×SkillPower/(DEF+100)` の素点が現実的ステータス帯（ATK 58〜100 / DEF 32〜90）で常に1.0未満となり、`int(max(1, …))` で実質全打撃が1ダメージ化。再現実験では20戦全てが30ターン引き分け（ダメージ分布: 0が50回・1が3,770回）。結果、レーティング更新が一度も発火せずランク戦/ランキングが死文化。docs/05はこの式を「確定」としつつログ例では32〜41ダメージを提示しており自己矛盾。既存テストはATK=250（STAT_MAX超）で決着可能性を検証しておりバグを隠蔽。修正候補: 係数K導入（K=60〜80でログ例と整合）等。docs/05・テストの同時更新必須。※ Issue #23 |
